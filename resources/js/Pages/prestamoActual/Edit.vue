...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.prestamoActuala[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.prestamoActuala[names['order']]
        });

        // form.codigo = props.prestamoActuala?.codigo
    }
})

const update = () => {
    form.put(route('prestamoActual.update', props.prestamoActuala?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.prestamoActuala.prestamoActual_id]"></v-select>
